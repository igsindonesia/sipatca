...
class PublikasiController extends Controller
{
    function index(Request $request) {
        $query = Submission::where('type', Submission::TYPES[15]);
        $submissionType = SubmissionOrderingService::getSubmissionTypeString(15);
        $submissions = SubmissionOrderingService::applyOrdering($query, $submissionType)->paginate(10);
        return view('admin.lecturer.surat-tugas.publikasi.index', compact('submissions'));
    }

    function show(Request $request, Submission $submission) {
        return view('admin.lecturer.surat-tugas.publikasi.show', compact('submission'));
    }
    ...
    function update(Request $request, Submission $submission) {
        ...
        return redirect()->route('admin.surat-lainnya-dosen.publikasi.index')->with([
            'status' => 'success',
