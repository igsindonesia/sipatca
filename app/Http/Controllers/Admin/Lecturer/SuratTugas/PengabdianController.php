...
namespace App\Http\Controllers\Admin\Lecturer\SuratTugas;
...
class PengabdianController extends Controller
{
    function index(Request $request) {
        $query = Submission::where('type', Submission::TYPES[14]);
        $submissionType = SubmissionOrderingService::getSubmissionTypeString(14);
        $submissions = SubmissionOrderingService::applyOrdering($query, $submissionType)->paginate(10);
        return view('admin.lecturer.surat-tugas.pengabdian.index', compact('submissions'));
    }

    function show(Request $request, Submission $submission) {
        return view('admin.lecturer.surat-tugas.pengabdian.show', compact('submission'));
    }
    ...
    function update(Request $request, Submission $submission) {
        ...
        return redirect()->route('admin.surat-lainnya-dosen.pengabdian.index')->with([
            'status' => 'success',
