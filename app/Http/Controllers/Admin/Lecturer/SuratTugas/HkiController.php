...
class HkiController extends Controller
{
    function index(Request $request) {
        $query = Submission::where('type', Submission::TYPES[13]);
        $submissionType = SubmissionOrderingService::getSubmissionTypeString(13);
        $submissions = SubmissionOrderingService::applyOrdering($query, $submissionType)->paginate(10);
        return view('admin.lecturer.surat-tugas.hki.index', compact('submissions'));
    }

    function show(Request $request, Submission $submission) {
        return view('admin.lecturer.surat-tugas.hki.show', compact('submission'));
    }
    ...
    function update(Request $request, Submission $submission) {
        ...
        return redirect()->route('admin.surat-lainnya-dosen.hki.index')->with([
            'status' => 'success',
