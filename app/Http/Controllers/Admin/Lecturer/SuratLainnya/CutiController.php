...
namespace App\Http\Controllers\Admin\Lecturer\SuratLainnya;
...
class CutiController extends Controller
{
    function index(Request $request) {
        $query = Submission::where('type', Submission::TYPES[12]);
        $submissionType = SubmissionOrderingService::getSubmissionTypeString(12);
        $submissions = SubmissionOrderingService::applyOrdering($query, $submissionType)->paginate(10);
        return view('admin.lecturer.surat-lainnya.cuti.index', compact('submissions'));
    }

    function show(Request $request, Submission $submission) {
        return view('admin.lecturer.surat-lainnya.cuti.show', compact('submission'));
    }
    ...
    function update(Request $request, Submission $submission) {
        ...
        return redirect()->route('admin.surat-lainnya-dosen.cuti.index')->with([
            'status' => 'success',
